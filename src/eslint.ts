...
function catalogHash(repoRoot: string): string {
  try {
    return sha256(
      fs.readFileSync(
        Path.join(repoRoot, 'scripts', 'path.ux', 'generated', 'api-paths.json'),
        'utf-8',
      ),
    );
  } catch {
...
function resultCacheKey(parts: {
  ...
  return sha256(
    [configHash, catalogHash, eslintVersion, argsKey, relPath, sha256(content)].join(' '),
  );
}
...
function formatResult(result: EslintJsonResult): string {
  ...
  for (const m of result.messages) {
    ...
    lines.push(
      `  ${m.line ?? 0}:${m.column ?? 0}  ${severity}  ${m.message}${m.ruleId ? '  ' + m.ruleId : ''}`,
    );
  }
  lines.push('');
  return lines.join('\n') + '\n';
...
export async function run(targetPath: string, rawEslintArgs: string[]) {
  ...
  for (let i = 0; i < toLint.length; i++) {
    ...
    const size = fs.statSync(toLint[i]!).size;
